    ...
    <div class="row justify-content-center g-4">
        ...
        <div class="col-md-4 col-sm-6">
            <div class="carrier-card p-4 text-center text-white shadow-lg" 
                 ...
                 onclick="selectCarrier(<?= $c['id'] ?>, '<?= htmlspecialchars($c['name']) ?>', '<?= $c['color'] ?>')">
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="from-search" placeholder="Начните вводить адрес или выберите на карте" readonly>
                        <div class="office-list" id="from-office-list" style="display:none;"></div>
                    </div>
                    
                    <div class="mb-3">
                        <label class="form-label fw-bold">Отделение получения:</label>
                        <input type="text" class="form-control" id="to-search" placeholder="Начните вводить адрес или выберите на карте" readonly>
                        <div class="office-list" id="to-office-list" style="display:none;"></div>
                    </div>
            ...
            .then(data => {
                ...
                if (!fromOffice) {
                    if (confirm(`Вы хотите выбрать это место как отделение отправления?\n${address}`)) {
                        document.getElementById('from-search').value = address;
                        fromOffice = { lat: lat, lng: lng, address: address };
                    }
                } else if (!toOffice) {
                    if (confirm(`Вы хотите выбрать это место как отделение получения?\n${address}`)) {
                        document.getElementById('to-search').value = address;
                        toOffice = { lat: lat, lng: lng, address: address };
                    }
                }
            })
            .catch(error => {
                        ...
                        .on('click', function() {
                            if (!fromOffice) {
                                if (confirm(`Выбрать это отделение как отправление?\n${office.city}, ${office.address}`)) {
                                    document.getElementById('from-search').value = `${office.city}, ${office.address}`;
                                    fromOffice = office;
                                }
                            } else if (!toOffice) {
                                if (confirm(`Выбрать это отделение как получение?\n${office.city}, ${office.address}`)) {
                                    document.getElementById('to-search').value = `${office.city}, ${office.address}`;
                                    toOffice = office;
                                }
                            }
                        });
...
function calculateRoute() {
    ...
    if (typeof fromOffice.lat === 'string') {
        fromLat = parseFloat(fromOffice.lat);
        fromLng = parseFloat(fromOffice.lng);
    } else {
        fromLat = fromOffice.lat;
        fromLng = fromOffice.lng;
    }
    
    if (typeof toOffice.lat === 'string') {
        toLat = parseFloat(toOffice.lat);
        toLng = parseFloat(toOffice.lng);
    } else {
        toLat = toOffice.lat;
        toLng = toOffice.lng;
    }
    ...
    routingControl.on('routesfound', function(e) {
        ...
        fetch(`get_carrier.php?id=${carrierId}`)
            .then(response => response.json())
            .then(carrier => {
